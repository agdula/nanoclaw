...
function parseArgs(args: string[]): RegisterArgs {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--assistant-name': result.assistantName = args[++i] || 'Jan'; break;
    }
  }
...
export async function run(args: string[]): Promise<void> {
  ...
  if (parsed.assistantName !== 'Jan') {
    logger.info({ from: 'Jan', to: parsed.assistantName }, 'Updating assistant name');
    ...
    for (const mdFile of mdFiles) {
      if (fs.existsSync(mdFile)) {
        ...
        content = content.replace(/^# Jan$/m, `# ${parsed.assistantName}`);
        content = content.replace(/You are Jan/g, `You are ${parsed.assistantName}`);
