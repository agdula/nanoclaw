...
describe('credentials detection', () => {
  it('detects ANTHROPIC_API_KEY in env content', () => {
    ...
    const hasCredentials = /^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY|GITHUB_TOKEN|GH_TOKEN|GITHUB_COPILOT_TOKEN)=/m.test(content);
  ...
  it('detects GitHub Copilot token in env content', () => {
    const content = 'GITHUB_COPILOT_TOKEN=ghu_test123';
    const hasCredentials = /^(CLAUDE_CODE_OAUTH_TOKEN|ANTHROPIC_API_KEY|GITHUB_TOKEN|GH_TOKEN|GITHUB_COPILOT_TOKEN)=/m.test(content);
    expect(hasCredentials).toBe(true);
  });
